	...
	constructor(
		...
		editorResolverService.registerEditor(
			`${GettingStartedInput.RESOURCE.scheme}:/**`,
			{
				...
				label: localize("welcome.displayName", "MudLabs Welcome"),
...
function isStartupPageEnabled(
	...
	if (startupEditor.value !== startupEditor.userRemoteValue) {
		logService.trace(
			`Startup editor is configured to be "${startupEditor.value}". This setting will be overridden by "${startupEditor.userRemoteValue}".`
		);
	}
